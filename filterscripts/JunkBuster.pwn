...
#define JB_Speed(%0,%1,%2,%3,%4) 				floatround(floatsqroot((%4)?(%0*%0+%1*%1+%2*%2):(%0*%0+%1*%1))*%3*1.7)
...
static
	...
	Text: JB::KickBanBox[6] = {Text: INVALID_TEXT_DRAW, ...},
...
PUBLIC: VerifyNoReload (playerid)
{
	...
	{
		JB::Kick (playerid, "Not reloading (Sawn-off Shotgun)");
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::LogEx ("%s buvo uþtildytas %d sec. uþ %s.", JB::PlayerInfo [playerid][JB::pName], time, reason);
		JB::SendFormattedMessageToAll (JB_RED, "||{666666} Þaidëjas '%s' (%d) uþtildomas %d sec. Prieþastis: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	...
	{
	    if(!strcmp(reason, "n", true)){ Kick (playerid); return 1; }
		...
		JB::SendFormattedMessageToAll (JB_RED, "||{666666} Ið serverio iðmestas þaidejas '%s'. Prieþastis: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "||{666666} '%s' uþbanintas uþ: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		for(new i = 0; i < 6; i++)
		{
			if (JB::KickBanBox[i] == Text: INVALID_TEXT_DRAW)
			{
			    switch(i)
				{
				    case 0: {
						JB::KickBanBox[i] = TextDrawCreate(336.000, 333.178, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -4.546);
						TextDrawTextSize(JB::KickBanBox[i], 361.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
						TextDrawFont(JB::KickBanBox[i], 0);
					}
					case 1: {
					    JB::KickBanBox[i] = TextDrawCreate(358.500, 307.545, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -4.546);
						TextDrawTextSize(JB::KickBanBox[i], 383.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
					...
					case 2: {
                        JB::KickBanBox[i] = TextDrawCreate(381.000, 279.673, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -3.647);
						TextDrawTextSize(JB::KickBanBox[i], 398.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
					...
					case 3: {
                        JB::KickBanBox[i] = TextDrawCreate(397.500, 258.519, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -2.697);
						TextDrawTextSize(JB::KickBanBox[i], 409.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
					...
					case 4: {
                        JB::KickBanBox[i] = TextDrawCreate(321.000, 351.605, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -3.447);
						TextDrawTextSize(JB::KickBanBox[i], 337.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
					...
					case 5: {
                        JB::KickBanBox[i] = TextDrawCreate(310.500, 363.305, "box");
						TextDrawLetterSize(JB::KickBanBox[i], 0.000, -2.596);
						TextDrawTextSize(JB::KickBanBox[i], 321.000, 0.000);
						TextDrawAlignment(JB::KickBanBox[i], 1);
						TextDrawColor(JB::KickBanBox[i], 0);
						TextDrawUseBox(JB::KickBanBox[i], 1);
						TextDrawBoxColor(JB::KickBanBox[i], -215);
		...
		for(new i = 0; i < 6; i++) TextDrawShowForPlayer (playerid, JB::KickBanBox[i]);
		
		if (JB::KickBanTitle == Text: INVALID_TEXT_DRAW)
		{
	        JB::KickBanTitle = TextDrawCreate (320.000000, 100.000000, ban ? ("~n~BANNED~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("~n~KICKED~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
			TextDrawAlignment (JB::KickBanTitle, 2);
			TextDrawBackgroundColor (JB::KickBanTitle, 255);
			TextDrawFont (JB::KickBanTitle, 2);
			TextDrawLetterSize (JB::KickBanTitle, 0.400000, 1.75);
			TextDrawColor (JB::KickBanTitle, -16776961);
			TextDrawSetOutline (JB::KickBanTitle, 1);
			TextDrawSetProportional (JB::KickBanTitle, 1);
			TextDrawUseBox (JB::KickBanTitle, 0);
			TextDrawBoxColor (JB::KickBanTitle, 96);
			TextDrawTextSize (JB::KickBanTitle, 0.000000, 130.000000);
		}
		else
		    TextDrawSetString (JB::KickBanTitle, ban ? ("~n~BANNED~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("~n~KICKED~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
		TextDrawShowForPlayer (playerid, JB::KickBanTitle);

		if (JB::KickBanInfo == Text: INVALID_TEXT_DRAW)
		{
		    JB::KickBanInfo = TextDrawCreate (275.000000, 155.000000, "_");
			TextDrawBackgroundColor (JB::KickBanInfo, 255);
			TextDrawFont (JB::KickBanInfo, 1);
			TextDrawLetterSize (JB::KickBanInfo, 0.4, 1.75);
			TextDrawColor (JB::KickBanInfo, -1);
			TextDrawSetOutline (JB::KickBanInfo, 1);
			TextDrawSetProportional (JB::KickBanInfo, 1);
			TextDrawSetShadow (JB::KickBanInfo, 1);
		}
		format (string, sizeof (string), "~y~Vardas:~n~~w~%s~n~~y~Data:~n~~w~%s~n~~y~Laikas:~n~~w~%s~n~~y~IP:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
			JB::PlayerInfo [playerid][JB::pIp]);
		TextDrawSetString (JB::KickBanInfo, string);
		TextDrawShowForPlayer (playerid, JB::KickBanInfo);
		format(string,sizeof(string),"Prieþastis: %s", reason);
		SendClientMessage(playerid, 0x00D91DFF, string);
		...
		if (JB::KickBanHelp == Text: INVALID_TEXT_DRAW)
		{
			...
			TextDrawLetterSize (JB::KickBanHelp, 0.2, 0.875);
			TextDrawColor (JB::KickBanHelp, -1);
			TextDrawSetOutline (JB::KickBanHelp, 1);
			TextDrawSetProportional (JB::KickBanHelp, 1);
			TextDrawSetShadow (JB::KickBanHelp, 1);
			TextDrawUseBox (JB::KickBanHelp, 0);
			TextDrawBoxColor (JB::KickBanHelp, 0);
			TextDrawTextSize (JB::KickBanHelp, 2.000000, 243.000000);
		}
		format (string, sizeof (string), "Jeigu buvai neteisingai %s, praðome paspausti F8 kad padarytumëte nuotraukà ir uþpildytumëte anketà %s.", ban ? ("uþbanintas") : ("iðmestas"), JB::Homepage);
		SendClientMessage(playerid, 0xb791bcFF, string);
...
PRIVATE: AdvertisementCheck (text [])
{
	if (!isnull (text))
	{
		...
		if (!strfind (string, "www.", false) || !strfind (string, "http://", false) || !strfind (string, ".com", false) || !strfind (string, ".net", false)
			|| !strfind (string, ".lt", false) || !strfind (string, ".org", false))
			return true;
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		JB::Kick (playerid, "Reklama.");
		return 1;
	}

	if (JB::Variables [CAPS_LOCK] && CapsLockCheck (text))
	{
		SendClientMessage (playerid, JB_RED, "||{666666} You are not allowed to use Caps Lock! Press [Caps Lock] to disable it.");
		return 1;
	}

	if (JB::Variables [SINGLEPLAYER_CHEATS] && SingleplayerCheatCheck (text))
	{
		JB::Kick (playerid, "Kodai-SAMP'e-Neveikia!");
		return 1;
	}

	if (JB::Variables [BAD_WORDS] && BadWordsCheck (text))
	{
		SendClientMessage (playerid, JB_RED, "||{666666} Nesikeik! Gerbk save ir kitus!");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "||{666666} Pastebëtas þaidëjas. %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
        ...
        if (JB::Variables [MAX_TOTAL_WARNINGS])
        {
			...
			if (totalwarnings >= JB::Variables [MAX_TOTAL_WARNINGS])
			{
			    switch (JB::Variables [TOO_MANY_WARNS_ACTION])
			    {
			        case 1:
					{
					    JB::Kick (i, "Follower of Dark Magic.");
					    continue;
					}

					case 2:
					{
					    JB::Ban (i, "Follower of Dark Magic.");
					    continue;
					}

					case 3:
					{
					    TempBan (i, 1, "Follower of Dark Magic.");
					    continue;
					}
			    }
			}
	    ...
	    ForEachPlayer (i)
	    {
	        if (!JB::SyncInfo [i][SYNC_TYPE_POS][JB::sSyncTime] && DOB::GetBit (JB::FullyConnected, i))
	        {
		        if (GetPlayerSurfingVehicleID (i) == INVALID_VEHICLE_ID && GetPlayerSurfingObjectID (i) == INVALID_OBJECT_ID && JB::GetPlayerSpeed (i, true) < 30 && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER || JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT))
		        {
					...
					if (speed > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pLastAirbrakeSpeed] > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pCurrentState] != PLAYER_STATE_DRIVER)
					{
					...
					else if (speed - 10 > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pLastAirbrakeSpeed] > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER)
					{
					    JB::Warnings [i]{AIRBRAKE}++;
						if (JB::Warnings [i]{AIRBRAKE} < MAX_CHECKS)
						{
			...
			if (DOB::GetBit (JB::FullyConnected, i) && JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER && !JB::IsPlayerAdmin (i))
			{
				...
				if (speed > 15 && abs(JB::PlayerInfo [i][JB::pOldSpeed] - speed) < 25.0 && (170.0 < ad < 190.0))
					if ((x < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][0] < 0.0) && (y < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][1] < 0.0) && (z < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][2] < 0.0))
						JB::Ban (i, "Quick turn (Neámanomai greitas apsisukimas)"); //He must have used quick turn! I think there is no other way to satisfy the statements above...
	...
	ForEachPlayer (i)
	{
		if (DOB::GetBit (JB::FullyConnected, i) && GetPlayerPos (i, x, y, z))
		{
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    ...
			    {
			        JB::Ban (i, "Praðome nenaudoti paðaliniø programø.");
			        continue;
			    }
			    ...
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
				        if (++JB::Warnings [i]{JOYPAD} >= JB::Variables [JOYPAD])
				        	JB::Kick (i, "Naudoja joypad");
						else
						{
						    JB::PlayerInfo [i][JB::pFreezeTime] = 5; // Freeze for 5 seconds.
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, "||{666666} Please stop using a joypad/gamepad. Use your keyboard and mouse.");
						}
				    }
				}
			        ...
			        if (++JB::PlayerInfo [i][JB::pBuyingGuns] >= 2)
			        {
				        if (GetTickCount () - JB::PlayerInfo [i][JB::pLastMoneyChange] > 5000)
				        {
				            if (++JB::Warnings [i]{PAY_FOR_GUNS} >= JB::Variables [PAY_FOR_GUNS])
				            {
				                ...
				                format (reason, sizeof (reason), "Nesumokëjo uþ ginklà: %s", reason);
					            JB::Kick (i, reason);
					            continue;
				            }
				        }
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, JB_RED, "JunkBuster:{666666} Please fix your framerate (FPS) or you will get kicked! (Min %d, your FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "JunkBuster:{666666} Pressing F7 once may help to fix it. (Removes the outlines of the letters in chat window.)");
						}
					}
				}
						...
						if (JB::PlayerInfo [i][JB::pCurrentState] != PLAYER_STATE_PASSENGER && !surfing && !IsPlayerInRangeOfPoint (i, 350.0, JB::PlayerInfo [i][JB::pCurrentPos][0], JB::PlayerInfo [i][JB::pCurrentPos][1], JB::PlayerInfo [i][JB::pCurrentPos][2]))
						{
							if (!IsPlayerInRangeOfPoint (i, 5.0, JB::PlayerInfo [i][JB::pSetPos][0], JB::PlayerInfo [i][JB::pSetPos][1], JB::PlayerInfo [i][JB::pSetPos][2]))
							{
								...
								if (JB::Variables [TELEPORT_HACK])
								{
									...
									{
										format (reason, sizeof (reason), "Teleport'ai: %s", reason);
										JB::Ban (i, reason);
										continue;
									}
				...
				if (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER)
				{
					...
					if (JB::Variables [TUNING_HACK])
					{
					    for (new j = 0; j < MAX_COMPONENT_SLOTS; ++j)
					    {
					        ...
					        {
					            JB::Kick (i, "Tuning hack'as. Tokiais reikalais kreipkis á mechanikus.");
					            SetVehicleToRespawn (vehicleid);
					            continue;
					        }
					    }
							    ...
							    if (75.0 < abs (rz) < 105.0)
							    {
									...
									if (floatround (rz * 160.0) < JB::Variables [WALLRIDE] && var > 10)
									{
									    ...
									    if (JB::PlayerInfo [i][JB::pWallriding] == MAX_CHECKS)
									    {
										    ...
										    {
										        JB::Kick (i, "Kiaulë voras. (Spider wheels)");
										        continue;
										    }
										    else
										    {
				...
				if (IsWeaponForbiddenForPlayer (i, var))
				{
					...
					format (reason, sizeof (reason), "Nelegalûs ginklai (%s)", reason);
					JB::Ban (i, reason);
					continue;
				}
				...
				if (JB::Variables [NO_RELOAD_SAWNOFF])
				{
					if (var == 26 && JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT)
					{
						...
						if (ivar [0] > 5 && ivar [0] < 11)//6 - 10 shots per second with sawn-off is cheating!
						{
							...
							{
								JB::Ban (i, "Not reloading (Sawn-off Shotgun)");
								continue;
							}
						}
				...
				if (JB::PlayerInfo [i][JB::pPingCheckProgress] == MAX_PING_CHECKS)
				{
					if (JB::Variables [MAX_PING])
					{
						...
						{
							format (reason, sizeof (reason), "Per didelis ping'as. [%d/%d]", var, JB::Variables [MAX_PING]);
							JB::Kick (i, reason);
							continue;
						}
					}
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "||{666666} Tu esi uþbanintas %dd, %dh, %dmin ir %dsec!", days, hours, minutes, seconds);
					JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					BanEx (playerid, "Tu esi uþbanintas..");
					return 1;
				}
				else
	...
	PUBLIC: TempBan (playerid, days, reason [])
	{
		if (days > 0)
		{
		    if (TempBanCount < sizeof (TempBanInfo))
		    {
				...
				++TempBanCount;
				format (string, sizeof (string), "%s [%dd.]", reason, days);
		...
		&& GetVehicleDistanceFromPoint (vehicleid, JB::VehiclePos [vehicleid][0], JB::VehiclePos [vehicleid][1], JB::VehiclePos [vehicleid][2]) > 50.0)
	{
	    new Float: x, Float: y, Float: z;
 		...
 		if (GetVehicleDistanceFromPoint (vehicleid, x, y, z) < 5.0)
 		{
 		    ...
 		    {
 		        new reason [64] = "Maðinos teleportavimas: ";
	    ...
	    if (JB::PlayerInfo [playerid][JB::pLastVehicle] != 0 && vehicleid != 0 && !JB::IsPlayerAdmin (playerid))
	    {
	        if (JB::PlayerInfo [playerid][JB::pLastVehicle] != vehicleid)
	        {
			    if (!JB::SyncInfo [playerid][SYNC_TYPE_POS][JB::sSyncTime])
			    {
					...
					{
				        JB::Ban (playerid, "Teleportavimasis maðinose");
		    ...
		    else if (JB::Variables [VEHICLE_REPAIR] && !JB::SyncInfo [playerid][SYNC_TYPE_VEHICLE][JB::sSyncTime])
		    {
		        if (fvar > JB::PlayerInfo [playerid][JB::pVehicleHealth])
		        {
		            ...
		            SendClientMessage(playerid, JB_RED, "||{666666}Tam yra {BBBBBB}mechanikai{666666}.. Sekantis bandymas magiðkai taisyti maðinà baigsis banu 2h.");
		            SetVehicleHealth(vehicleid, JB::PlayerInfo [playerid][JB::pVehicleHealth]);
		            if (JB::Warnings [playerid]{VEHICLE_REPAIR} >= JB::Variables [VEHICLE_REPAIR])
		            {
		                SetVehicleHealth(vehicleid, JB::PlayerInfo [playerid][JB::pVehicleHealth]);
		                JB::Kick (playerid, "Maðinos taisymas su burtø lazdele.");
		                return 0;
		            }
		            else
			...
			if (JB::PlayerInfo [playerid][JB::pOldWeapon] == ivar)//same weapon.
			{
				...
				else
				{
					...
					if (JB::PlayerInfo [playerid][JB::pAmmoUsed][ivar] > (AmmoAmount [ivar] * 2) && ammoused > 0)//Player must have switch weapons fast or doesn't reload at all
					{
						...
						{
							GetWeaponName (ivar, reason, sizeof (reason));
							format (reason, sizeof (reason), "No reload (%s)", reason);
							JB::Kick (playerid, reason);
							return 0;
						}
					}
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
				    if (++JB::Warnings [killerid]{DRIVE_BY} >= MAX_CHECKS)
				        JB::Kick (killerid, "Drive-by");
					else
					{
						JB::SetPlayerHealth (killerid, 0.0);
						SendClientMessage (killerid, JB_RED, "||{666666} Ðaudei ir pataikei su kulka sau á kaktà! Mldc!");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
						    ...
						    format (string, sizeof (string), "Masinis þudymas su %s", JB::GetVehicleName (vehicleid));
						    JB::Kick (killerid, string);
						}
						else
							JB::SendFormattedMessage (killerid, JB_RED, "||{666666} Þudymas su %s yra neleidþiamas! (Áspëjimai %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						JB::Kick (killerid, "Masinis þudymas spawne");
					else
						JB::SendFormattedMessage (killerid, JB_RED, "||{666666} Neþudyk spawne! (Áspëjimai %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
...
CALLBACK: OnFilterScriptInit ()
{
	...
	if ((JB::AirbrakeDetection == JB::Variables [AIRBRAKE_DETECTION])) // start timer only when fast mode is activated.
	    SetTimer ("AirbrakeCheck", 100, true);
	...
	JB::Log (" ---:: JunkBuster (v"#JB_VERSION")");
	return 0;
}
...
CALLBACK: OnFilterScriptExit ()
{
	...
	for(new i = 0; i < 6; i++)
    if (JB::KickBanBox[i] != Text: INVALID_TEXT_DRAW)
		TextDrawDestroy (JB::KickBanBox[i]);
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "||{666666} Tu uþbanintas.. Apsilankyk http://www.soc.lt");
						JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "||{666666} Tu laikinai uþdabintas dar %dd, %dh, %dmin ir %dsec! Apsilankyk http://www.soc.lt", days, hours, minutes, seconds);
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "||{666666} Tu esi uþtildytas dar %dsec!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
	...
	if (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY])
	{
		...
		if (!JB::PlayerInfo [playerid][JB::pMuted] && JB::Variables [SPAM])
		{
			if (JB::PlayerInfo [playerid][JB::pMessages] > 30 && !JB::IsPlayerAdmin (playerid))
			{
				JB::Ban (playerid, "Spameris");
				return 0;
			}
			else if (JB::PlayerInfo [playerid][JB::pMessages] > 20)
			{
				JB::Kick (playerid, "Spameris");
				return 0;
			}
			else if (JB::PlayerInfo [playerid][JB::pMessages] > 10 || JB::PlayerInfo [playerid][JB::pMessageRepeated] >= MAX_CHECKS)
			{
				JB::PlayerInfo [playerid][JB::pMessageRepeated] = 0;
				JB::MutePlayer (playerid, 60, "Baik spamint!");
				return 0;
			}
		}
...
COMMAND:fps (playerid, params [])
{
	...
	if (!JB::Variables [MIN_FPS])
		return SendClientMessage (playerid, JB_RED, "Ði funkcija yra iðjungta!");

	JB::SendFormattedMessage (playerid, JB_GREEN_BLUE, "Tavo FPS: %d", JB::GetPlayerFPS (playerid));
	return 1;
}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} JunkBuster Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} JunkBuster Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Player '%s' has successfully been added to blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
				...
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not remove player '%s' from blacklist!", params);
			return 1;
	}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not add player '%s' to whitelist!", params);
			return 1;
	}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not remove player '%s' from whitelist!", params);
			return 1;
	}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} JunkBuster variable '%s' has successfully been set to %d.", var, value);
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    SendClientMessage (playerid, JB_RED, "JunkBuster:{666666} Sorry, this command is currently not available when using JunkBuster database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not ban IP %s!", params);
				return 1;
			}
		}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{666666} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
			...
			else if (oldstate == PLAYER_STATE_ONFOOT)
			{
			    if (!IsPlayerInRangeOfPoint (playerid, 20.0, JB::PlayerInfo [playerid][JB::pOnFootPos][0], JB::PlayerInfo [playerid][JB::pOnFootPos][1], JB::PlayerInfo [playerid][JB::pOnFootPos][2]))
			    {
			        ...
			        if (JB::Variables [VEHICLE_TELEPORT] && JB::Warnings [playerid]{VEHICLE_TELEPORT} >= MAX_CHECKS)
						JB::Kick (playerid, "Maðinos teleportavimas");
			        else
			            JunkBusterReport (playerid, "vehicle teleport", JB::GetVehicleName (vehicleid));
			    }
			}
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "JunkBuster:{666666} Please stop performing the C-Bug or you will get kicked/banned.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "||{666666} Nedaryk drive-by!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_CFG:
			{
				if (response)
				{
					switch (listitem)
					{
						...
						{
							ConfigJunkBuster ();
							SendClientMessage (playerid, JB_GREEN, "JunkBuster:{666666} Configuration has been loaded from file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							SaveJunkBusterVars ();
							SendClientMessage (playerid, JB_GREEN, "JunkBuster:{666666} Configuration has been saved to file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							JB::Variables = JB::DefaultVariables;
							SendClientMessage (playerid, JB_GREEN, "JunkBuster:{666666} Default configuration has been loaded.");
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{666666} JunkBuster variable '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
