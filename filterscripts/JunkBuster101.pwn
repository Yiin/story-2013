...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "||{FFFF00} Þaidëjas '%s' (%d) uþtildomas %d sec. Reason: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		else
		{
			...
			format (string, sizeof (string), "~r~Iðmetimo prieþastis: ~n~%s", reason);
			GameTextForPlayer (playerid, string, 60000, 4);
		}

		TogglePlayerControllable (playerid, false);
		//SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "||{FFFF00} Ið serverio iðmestas þaidejas '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) buvo iðmestas uþ %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		else
		{
			...
			format (string, sizeof (string), "~r~Ban prieþastis: ~n~%s", reason);
			GameTextForPlayer (playerid, string, 60000, 4);
		}

		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "||{FFFF00} Uþbanintas þaidëjas '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) buvo uþbanintas uþ %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, JB_RED, "||{FFFF00}Ájungtas Caps Lock! Spausk [Caps Lock] dar kartà kad já iðjungtum.");
		return 1;
	}

	if (JB::Variables [SINGLEPLAYER_CHEATS] && SingleplayerCheatCheck (text))
	{
		JB::Kick (playerid, "Nelogiðkas bandymas naudoti singleplayer kodus.");
		return 1;
	}

	if (JB::Variables [BAD_WORDS] && BadWordsCheck (text))
	{
		SendClientMessage (playerid, JB_RED, "||{FFFF00} Praðome nesikeikti!");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "[%s] %s (%d, %s) su %s, %s", JB::GetTime (), JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], report, details);
		...
		format (string, sizeof (string), "||{FFFF00} Átariamas þaidëjas dël %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
    ...
    ForEachPlayer (i)
    {
        ...
        if (DOB::GetBit (JB::FullyConnected, i))
        {
				...
				&& (nokeypressed || IsPlayerInRangeOfPoint (i, 3.0, JB::PlayerInfo [i][JB::pAFKPos][0], JB::PlayerInfo [i][JB::pAFKPos][1], JB::PlayerInfo [i][JB::pAFKPos][2])))
	        {
	            ...
	            {
	                format (reason, sizeof (reason), "Pasiektas AFK limitas [Max %d min(s)]", JB::Variables [AFK]);
	                JB::Kick (i, reason);
	            }
	        }
			else
			...
			if (DOB::GetBit (JB::FullyConnected, i) && JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER && !JB::IsPlayerAdmin (i))
			{
				...
				if (speed > 15 && abs(JB::PlayerInfo [i][JB::pOldSpeed] - speed) < 25.0 && (170.0 < ad < 190.0))
					if ((x < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][0] < 0.0) && (y < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][1] < 0.0) && (z < 0.0) != (JB::PlayerInfo [i][JB::pVelocity][2] < 0.0))
						JB::Kick (i, "Quick turn"); //He must have used quick turn! I think there is no other way to satisfy the statements above...
				    ...
				    if (JB::Variables [REPORT_MONEY_HACK] && !JB::IsPlayerAdmin (i))
				    {
                        if (!HasTimePassed (JB::PlayerInfo [i][JB::pLastUpdate], 2000))
                        {
                            ...
                            if (JB::Warnings [i]{MONEY_HACK} >= MAX_CHECKS)
                            {
								...
								{
	                                format (reason, sizeof (reason), "Turëjo bûti $%d bet rasta $%d", JB::PlayerInfo [i][JB::pMoney], var);
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			        ...
			        if (++JB::PlayerInfo [i][JB::pBuyingGuns] >= 2)
			        {
				        if (GetTickCount () - JB::PlayerInfo [i][JB::pLastMoneyChange] > 5000)
				        {
				            if (++JB::Warnings [i]{PAY_FOR_GUNS} >= JB::Variables [PAY_FOR_GUNS])
				            {
				                ...
				                format (reason, sizeof (reason), "Nesumokëjai uþ ginklà: %s", reason);
					            JB::Kick (i, reason);
					            continue;
				            }
				        }
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							format (reason, sizeof (reason), "Per maþas FPS (%d, min %d)", var, JB::Variables [MIN_FPS]);
							JB::Kick (i, reason);
							continue;
						}
						else if (JB::Warnings [i]{MIN_FPS} % (MAX_CHECKS * 10) == 0)
						{
							JB::SendFormattedMessage (i, JB_RED, "||{FFFF00} Praðau, susitvarkyk lag'us (FPS) arba bûsi iðmestas!.. (Min %d, tavo FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "||{FFFF00} F7 paspaudimas vienà kartà gali padëti ðià bëdà iðspræsti. (Panaikins outlines nuo raidþiø chat lange.)");
						}
					}
				}
						...
						if (JB::PlayerInfo [i][JB::pCurrentState] != PLAYER_STATE_PASSENGER && !surfing && !IsPlayerInRangeOfPoint (i, 350.0, JB::PlayerInfo [i][JB::pCurrentPos][0], JB::PlayerInfo [i][JB::pCurrentPos][1], JB::PlayerInfo [i][JB::pCurrentPos][2]))
						{
							if (!IsPlayerInRangeOfPoint (i, 5.0, JB::PlayerInfo [i][JB::pSetPos][0], JB::PlayerInfo [i][JB::pSetPos][1], JB::PlayerInfo [i][JB::pSetPos][2]))
							{
								...
								if (JB::Variables [TELEPORT_HACK])
								{
									...
									{
										format (reason, sizeof (reason), "Teleport èytai : %s", reason);
										JB::Ban (i, reason);
										continue;
									}
									...
									else if (JB::Warnings [i]{TELEPORT_HACK} >= MAX_CHECKS)
									{
										...
										JunkBusterReport (i, "teleport èytai", reason);
								}
								else
									JunkBusterReport (i, "teleport èytai", reason);
							}
					...
					if (JB::Variables [AIRBRAKE] && !surfing && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT || (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER && JB::GetPlayerSpeed (i, true) < 30)))
					{
						...
						if (var >= JB::Variables [AIRBRAKE])
						{
							...
							if (JB::PlayerInfo [i][JB::pAirbraking] >= (MAX_CHECKS - 1))
							{
								...
								{
									format (reason, sizeof (reason), "aukðtis: %f, ~%d m/s, maðina: %s", z, var, JB::GetVehicleName (vehicleid));
						...
						if (JB::Variables [MAX_SPEED])
						{
							...
							if (!IsPlayerInPlane (i))
							{
								if (var > JB::Variables [MAX_SPEED])
								{
									...
									{
										format (reason, sizeof (reason), "%d KM/H su %s (Max %d)", var, JB::GetVehicleName (vehicleid), ivar [0]);
								    ...
								    if (75.0 < abs (rz) < 105.0)
								    {
										...
										if (floatround (rz * 160.0) < JB::Variables [WALLRIDE] && ivar [3] > 10)
										{
										    ...
										    if (JB::PlayerInfo [i][JB::pWallriding] == MAX_CHECKS)
										    {
											    ...
											    {
											        JB::Kick (i, "Kiaulë voras (Spider wheels hack)");
											        continue;
											    }
											    else
											    {
											        format (reason, sizeof (reason), "su %s", JB::GetVehicleName (vehicleid));
													JunkBusterReport (i, "Spider wheels hack", reason);
											    }
										    }
										}
								    }
				...
				if (JB::PlayerInfo [i][JB::pPingCheckProgress] == MAX_PING_CHECKS)
				{
					if (JB::Variables [MAX_PING])
					{
						...
						{
							format (reason, sizeof (reason), "Per didelis ping. [%d/%d]", var, JB::Variables [MAX_PING]);
							JB::Kick (i, reason);
							continue;
						}
					}
...
PRIVATE: SplitIp (ipstring [])
{
	...
	PUBLIC: BanIp (ipstring [])
	{
		...
		if (ip != 0xFFFFFFFF && IpBanCount < sizeof (IpBans))
		{
		    ...
		    JB::LogEx ("IP %s uzbanintas.", ipstring);
			SaveIpBans ();
			ForEachPlayer (j)
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "||{FFFF00} Tu laikinai uþbanintas %d dienai(-oms), %d valandoms(-ai), %d minutëms(-ei) ir %d sekundems(-ei)!", days, hours, minutes, seconds);
					JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
                    TempBan(playerid, days + 1, "Laikinai uþbanintas");
	...
	PUBLIC: TempBan (playerid, days, reason [])
	{
		if (days > 0)
		{
		    if (TempBanCount < sizeof (TempBanInfo))
		    {
				...
				++TempBanCount;
				format (string, sizeof (string), "%s [%d dienai (-oms)]", reason, days);
...
PRIVATE: ConfigJunkBuster ()
{
	...
	{
	    JB::Homepage = "sio serverio tinklapyje.";
	    ...
	    if (JB::PlayerInfo [playerid][JB::pLastVehicle] != 0 && vehicleid != 0 && !JB::IsPlayerAdmin (playerid))
	    {
	        if (JB::PlayerInfo [playerid][JB::pLastVehicle] != vehicleid)
	        {
			    if (!JB::PlayerInfo [playerid][JB::pUpdateCount])
			    {
					...
					{
				        JB::Ban (playerid, "Maðinø teleportavimas");
		    ...
		    else if (JB::Variables [VEHICLE_REPAIR] && !JB::PlayerInfo [playerid][JB::pRespraying])
		    {
		        if (fvar > JB::PlayerInfo [playerid][JB::pVehicleHealth])
		        {
		            ...
		            if (JB::Warnings [playerid]{VEHICLE_REPAIR} >= JB::Variables [VEHICLE_REPAIR])
		            {
		                ...
		                JB::Kick (playerid, "Maðinos taisymas su èytais");
		                return 0;
		            }
		            else
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						JB::Kick (killerid, "Spawn Kill");
					else
						JB::SendFormattedMessage (killerid, JB_RED, "||{FFFF00} Neþudyk þaidejø Spawn'e! (Warning %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
...
CALLBACK: OnFilterScriptInit ()
{
	...
	SetTimer ("JunkBuster", 200, true);
	SetTimer ("QuickTurnCheck", 200, true);
...
CALLBACK: OnPlayerConnect (playerid)
{
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "|{FFFF00} Tu uþbanintas.");
						JB::LogEx ("%s (%s) yra uþbanintas uþ bandymà iðvengti bano.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "||{FFFF00} Tu laikinai uþbanintas %d dienai(-oms), %d valandoms(-ai), %d minutëms(-ei) ir %d sekundems(-ei)!", days, hours, minutes, seconds);
			...
			{
			    DOB::SetBit (JB::KickBan, playerid, true);
				SendClientMessage (playerid, JB_RED, "||{FFFF00} Tu nesi èia laukiamas.");
				JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
				BanEx (playerid, "Tu nesi èia laukiamas..");
				return 0;
			}
	...
	{
		SendClientMessage (playerid, 0xae7b4eFF, "> Maloniai praðome þaisti sàþiningai, nes prieðingu atveju nukentësite tik jûs.");
	}
	return 1;
}
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "||{FFFF00} Negali raðyti dar %d sekundes(-æ) nes esi uþtildytas.", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
	...
	if (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY])
	{
		...
		if (!JB::PlayerInfo [playerid][JB::pMuted] && JB::Variables [SPAM])
		{
			...
			{
				JB::Ban (playerid, "Belenkoks spameris");
				return 0;
			}
			else if (JB::PlayerInfo [playerid][JB::pMessages] > 10)
			{
				JB::Kick (playerid, "Nespamink!");
				return 0;
			}
...
CALLBACK: OnPlayerCommandReceived (playerid, cmdtext [])
{
	...
	if (JB::Variables [COMMAND_SPAM] && (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY]))
	{
		...
		{
			JB::Ban (playerid, "Belenkoks komandø (/ ) spamas");
			return 0;
		}
		else if (JB::PlayerInfo [playerid][JB::pCommands] > 5)
		{
			JB::Kick (playerid, "Komandø (/ ) spamas");
			return 0;
		}
	}
	return 1;
...
COMMAND:myfps (playerid, params [])
{
	...
	if (!JB::Variables [MIN_FPS])
		return SendClientMessage (playerid, JB_RED, "Ði komanda nepasiekiama.");
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "||{FFFF00} JunkBuster Chrome sëkmingai surinko informacijà apie þaidejà '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "||{FFFF00} JunkBuster Chrome nepavyko surinkti pakankamai informacijos apie þaidejà '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "||{FFFF00} Homepage has been set to '%s'!", params);
	return 1;
}
			...
			else if (oldstate == PLAYER_STATE_ONFOOT)
			{
			    if (!IsPlayerInRangeOfPoint (playerid, 20.0, JB::PlayerInfo [playerid][JB::pOnFootPos][0], JB::PlayerInfo [playerid][JB::pOnFootPos][1], JB::PlayerInfo [playerid][JB::pOnFootPos][2]))
			    {
			        ...
			        if (JB::Variables [VEHICLE_TELEPORT] && JB::Warnings [playerid]{VEHICLE_TELEPORT} >= MAX_CHECKS)
						JB::Kick (playerid, "Maðinø teleportavimas");
			        else
			            JunkBusterReport (playerid, "vehicle teleport", JB::GetVehicleName (vehicleid));
			    }
			}
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "||{FFFF00} Baik naudoti C-Bug arba bûsi iðmestas/uþbanintas!");
...
/*	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "Praðau, nedaryk drive-by!");
				TogglePlayerControllable (playerid, false);
		...
		if (JB::Variables [CHECKPOINT_TELEPORT])
		{
			if (!JB::GetPlayerSpeed (playerid, false))
			{
				...
				else
				{
					...
					format (string, sizeof (string), "Checkpoint teleportas (%d áspëjimas)", JB::Warnings [playerid]{CHECKPOINT_TELEPORT});
					JunkBusterReport (playerid, "teleport hack", string);
				}
			}
		}
	}
