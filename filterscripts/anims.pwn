...
CMD:animhelp(playerid, params[])
{
	...
	Msg(playerid, -1, " | - /animlib {FFFF00}Open an animation library.");
	return 1;
}
CMD:animlib(playerid, params[])
{
	if(isnull(params)) return 1;
	PreloadAnimLib(playerid, params);
	
 	gCurrentLib[playerid][0] = EOS;
	strcat(gCurrentLib[playerid], params);
	
	new curlib, lib[32], anim[32];
	for(new i = 1;i<MAX_ANIMS;i++) // Loop through all animation IDs.
	{
	    GetAnimationName(i, lib, 32, anim, 32);
	    
	    if(strcmp(lib, params))
	    {
	        curlib++;
			break;
	        ...
	        ShowPlayerDialog(playerid, D_ANIM_LIST, DIALOG_STYLE_LIST, "Choose an animation", gAnimList[curlib], "Play", "Back");
	    }
	}
