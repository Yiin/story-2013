...
static Bars[MAX_PLAYER_BARS][E_BAR_DATA];
...
UpdateVars(index, Float:x, Float:y, Float:w, Float:h, color)
{
    ...
    Bars[index][pbar_w] = w;
    Bars[index][pbar_h] = h;
    Bars[index][pbar_colour] = color;
}

DeleteVars(index)
{
    Bars[index][pbar_x] = 0.0;
    Bars[index][pbar_y] = 0.0;
    Bars[index][pbar_w] = 0.0;
    Bars[index][pbar_h] = 0.0;
    Bars[index][pbar_colour] = 0;
}
...
public OnPlayerUpdate(playerid)
{
    if(GetPVarInt(playerid, "MovingBar"))
    {
        ...
        new PlayerBar:barid = PlayerBar:GetPVarInt(playerid, "BarID");
        ...
        if(ud || lr)
        {
            ...
            barid = CreatePlayerProgressBar(playerid,x, y, w, h, color, 100.0);
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    {
        DestroyPlayerProgressBar(playerid,PlayerBar:GetPVarInt(playerid, "BarID"));
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    if(!strcmp(cmdtext, "/bar", true))
    {
        ...
        {
            new PlayerBar:barid;
            if((barid = CreatePlayerProgressBar(playerid,320.0, 240.0, 56.50, 3.39, 0xFF0000FF, 100.0)) == INVALID_PLAYER_BAR_ID)
            {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    {
        SetPlayerProgressBarColor(playerid,PlayerBar:GetPVarInt(playerid, "BarID"), hexstr(inputtext));
        UpdatePlayerProgressBar(playerid,PlayerBar:GetPVarInt(playerid, "BarID"));
        Bars[GetPVarInt(playerid, "BarID")][pbar_colour] = hexstr(inputtext);
    }
    else if(dialogid == DIALOG_DONE)
    {
        ...
        format(line, sizeof line, "new PlayerBar:bar = CreatePlayerProgressBar(playerid,%.2f, %.2f, %.2f, %.2f, %d, 100.0);\r\n",
            Bars[barid][pbar_x], Bars[barid][pbar_y], Bars[barid][pbar_w], Bars[barid][pbar_h], Bars[barid][pbar_colour]);
